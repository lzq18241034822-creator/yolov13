...
def export_logits(model, loader, device, out_npz):
    ...
    print(f"[OK] saved student logits npz: {out_npz}")
...
def main():
    ...
    ap.add_argument("--out_npz", type=str, default="runs/stage2_student/student_logits.npz")
    ap.add_argument("--num_workers", type=int, default=0)
    ap.add_argument("--safe_loop", type=int, default=1)
    args = ap.parse_args()
    ...
    if args.safe_loop:
        ...
        np.savez_compressed(args.out_npz,
            ...
            chloroplast=np.concatenate(ch_logits,0),
            ids5=np.array(ids5_list, dtype=np.int32))
        print(f"[OK] saved student logits npz: {args.out_npz}")
    else:
